...
(function () {
  ...
  function mountPayments(root) {
    ...
    root.querySelector('[data-pay-create]').addEventListener('click', async () => {
      ...
      try {
        ...
        out.innerHTML = `<p><strong>Provider:</strong> ${d.provider}</p><p><strong>Invoice:</strong> ${d.invoiceNumber}</p><p><a href="${d.paymentUrl}" target="_blank" rel="noopener">Open payment link</a></p><p class="mvp-muted">Share this link on WhatsApp or open on a phone with a UPI app.</p>`;
        await refreshLedger();
      } catch (e) {
  ...
  function mountHrms(root) {
    async function loadEmployees() {
      ...
      sel.innerHTML = (d.employees || []).map((e) => `<option value="${e.id}">${e.name}</option>`).join('');
    }
  ...
  function mountPdf(root) {
    root.querySelector('[data-pdf-merge]').addEventListener('click', async () => {
      ...
      if (files.length < 2) {
        root.querySelector('[data-pdf-out]').textContent = 'Choose at least 2 PDF files.';
        return;
      }
      ...
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        root.querySelector('[data-pdf-out]').textContent = err.error || res.statusText;
        return;
      }
      const blob = await res.blob();
      const a = document.createElement('a');
      ...
      a.click();
      URL.revokeObjectURL(a.href);
      root.querySelector('[data-pdf-out]').textContent = 'Merged PDF downloaded.';
    });

    root.querySelector('[data-pdf-split]').addEventListener('click', async () => {
      const f = root.querySelector('[data-pdf-split-file]').files[0];
      if (!f) {
        root.querySelector('[data-pdf-out]').textContent = 'Choose one PDF.';
        return;
      }
      const fd = new FormData();
      fd.append('pdf', f);
      ...
      a.click();
      URL.revokeObjectURL(a.href);
      root.querySelector('[data-pdf-out]').textContent = 'Split PDF downloaded.';
    });
    ...
    root.querySelector('[data-pdf-extract]').addEventListener('click', async () => {
      ...
      root.querySelector('[data-pdf-out]').textContent = data.ok ? `Extracted ${data.chars || 0} characters.` : (data.error || 'Failed');
    });
  }
    ...
    if (key === 'pdf-studio') {
      return `
        <div class="mvp-grid">
          <section class="mvp-panel">
            <h3>Merge PDFs</h3>
            <p class="mvp-muted">Select 2+ PDFs. Download: sansa-merge.pdf</p>
