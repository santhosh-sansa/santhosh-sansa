...
async function consumeCredits(user, cost = 1) {
  ...
  try {
    ...
    warnNonCriticalWrite(error, 'credit file write');
  }
...
async function recordGeneration(userId, payload) {
  ...
  try {
    ...
    warnNonCriticalWrite(error, 'generation history file write');
  }
  return record;
}
...
async function createToneFile(prefix, options) {
  ...
  try {
    ...
    warnNonCriticalWrite(error, 'audio file write');
...
function buildAssistantReply(message) {
  ...
  const request = input.slice(0, 220);
  return [
    `SANSA workflow ready for: "${request}"`,
    '1. Creative: generate a launch poster brief with headline, Tamil/English copy, colors and image direction.',
    '2. Business: prepare invoice fields, GST notes, payment reminder copy and customer follow-up steps.',
    '3. Delivery: export the poster/PDF draft, copy WhatsApp text, and save the task in recent outputs.',
  ].join('\n');
}
